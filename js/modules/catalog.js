import catalogData from '../catalogo.data.js';

/**
 * Renderiza las categorías de equipos desde el módulo de datos del catálogo.
 */
export function initCatalog() {
  ...
  try {
    container.innerHTML = catalogData.categorias.map(renderCategory).join('');
    initDataSheetModal();
    populateFooterLinks(catalogData.categorias, footerEquipos);
  } catch (error) {
    console.error(error);
    container.innerHTML =
      '<p class="text-ascentra-gray text-center py-8">No se pudo cargar el catálogo.</p>';
  }
}
  ...
  return `
    <article class="product-card bg-white border border-gray-200 rounded-xl overflow-hidden flex flex-col" data-model="${equipo.nombre}">
      ...
      <div class="p-3 sm:p-5 flex flex-col flex-1">
        <h4 class="text-sm sm:text-lg font-black tracking-wide uppercase leading-tight mb-1">${equipo.nombre}</h4>
        <p class="text-ascentra-gray text-xs sm:text-sm mb-3 sm:mb-4">${equipo.descripcion}</p>
        <button
          type="button"
          data-ficha="${equipo.ficha_tecnica}"
          data-title="${equipo.nombre}"
          class="data-sheet-btn mt-auto btn-primary w-full bg-ascentra-yellow text-ascentra-dark font-bold text-[10px] sm:text-xs tracking-wide sm:tracking-widest uppercase px-2 sm:px-3 py-2.5 sm:py-3 rounded-full flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 leading-tight"
        >
          <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" aria-hidden="true">
